...
%     Q_t = Q_t+rand*10e-4*ones(size(Q_t));
    ...
    if mvFlag == 0
        [self_sub_p_hat_ts, p_fail, posteriors] = moveDetector(x+rand(size(x))*10e-3, gammaL, numMics, numArrays, micsPosNew, iter, p_fail, sub_p_hat_ts, scales, RTF_train,...
        rirLen, rtfLen, sourceTrain, holderTest, nL, nU, roomSize, T60, c, fs, kern_typ);
    else
        [self_sub_p_hat_ts, p_fail, posteriors] = moveDetector(x+rand(size(x))*10e-3, gammaL, numMics, numArrays, micsPosNew, iter, p_fail, self_sub_p_hat_ts, scales, RTF_train,...
        rirLen, rtfLen, sourceTrain, holderTest, nL, nU, roomSize, T60, c, fs, kern_typ);
    end
