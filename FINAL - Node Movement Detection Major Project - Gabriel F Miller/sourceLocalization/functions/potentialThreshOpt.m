function [tp_ch_curr,fp_ch_curr,tn_ch_curr,fn_ch_curr,posteriors] = potentialThreshOpt(radius_curr, thresh, sourceTrain, wavs, gammaL, T60, modelMean, modelSd, init_var, lambda, eMax, transMat, RTF_train, nL, nU,rirLen, rtfLen,c, kern_typ, scales, num_iters, roomSize, radiusU, ref, numArrays, mic_ref, micsPos, numMics, fs)
    tp_ch_curr = 0;
    fp_ch_curr = 0;
    tn_ch_curr = 0;
    fn_ch_curr = 0;
    ...
    for iters = 1:num_iters      
        ...
        [~, micsPosNew] = micNoRotate(roomSize, radius_curr, mic_ref, movingArray, micsPos, numArrays, numMics);
...
%             wav_folder = wavs(3:27);
      ...
      %---- Initialize subnet estimates of training positions ----
        ...
        [~, p_fail, posteriors_out] = moveDetectorOpt(x_tst, transMat, init_var, lambda, eMax, thresh, gammaL, numMics, numArrays, micsPosNew, 1, 0, sub_p_hat_ts, scales, RTF_train,...
                rirLen, rtfLen, sourceTrain, sourceTest, nL, nU, roomSize, T60, c, fs, kern_typ);

        posteriors_curr = posteriors_curr+posteriors_out;
    ...
    posteriors = posteriors_curr./num_iters;
