    ...
    for s = 1:num_snrs
        ...
        for ni = 1:num_iters
            ...
            for k = 1:numArrays
                ...
                [~,~,align_p_hat_ts(k,:)] = test(unk_noisy_x, gammaL, trRTF, subnet, rirLen, rtfLen, numArrays-1, numMics, sourceTrain,...
                    sourceTest, nL, nU, roomSize, T60, c, fs, kern_typ, subscales);  
                
                [subnet, ~, ~] = subNet(k, numArrays, numMics, scales, micsPosNew, RTF_train);
                [~,~,misalign_p_hat_ts(k,:)] = test(unk_noisy_x, gammaL, trRTF, subnet, rirLen, rtfLen, numArrays-1, numMics, sourceTrain,...
                    sourceTest, nL, nU, roomSize, T60, c, fs, kern_typ, subscales); 
            end
                   
            align_resids(t,count) = mean(mean(((pre_p_hat_ts-align_p_hat_ts).^2)));
            misalign_resids(t,count) = mean(mean(((pre_p_hat_ts-misalign_p_hat_ts).^2)));
            count = count+1;
        end
    end
end
