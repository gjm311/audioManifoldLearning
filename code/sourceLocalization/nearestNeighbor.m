...
for nl = 1:nL
    ...
    [costs, ~, ~, ~, scales_set] = grad_descent(sourceTrainL, numArrays, RTF_train, reshape(sigmaLs(nl,:,:), [nL,nL]), init_scales, var_init, max_iters, tol, alpha, kern_typ);
    [~,I] = min(sum(costs));
    scalesL(nl,:) = scales_set(I,:);
end
...
for s_iter = snrs+1
    ...
    for r = 1:numPer
        ...
        [~,~, p_hat_t] = test(unk_source, reshape(gammaLs(min_tRes_idx,:,:),[nL,nL]), RTF_train, micsPos, rirLen, rtfLen, numArrays,...
                        numMics, sourceTrain, sourceTest, nL, nU, roomSize, T60, c, fs, kern_typ, reshape(scalesL(min_tRes_idx,:),[1,5]));
    end
end
