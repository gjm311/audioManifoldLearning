...
for i = 1:num_iters
    acc_itr = accs(i).sims;
    if opt_err <= sum([acc_itr(1:end).latent_acc])
        opt_err = sum([acc_itr(1:end).pfail_acc]);
        opt_idx = i;
    end
end

opt_transMat = trans_mats(:,:,opt_idx);
opt_init_var = vars(opt_idx);
opt_eMax = eMaxs(opt_idx);
opt_lambda = lambdas(opt_idx);
opt_thresh = net_threshs(opt_idx);
save('mat_outputs/movementOptParams')
