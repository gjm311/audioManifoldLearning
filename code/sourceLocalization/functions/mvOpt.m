function [upd_sub_p_hat_ts, prob_failure, posteriors] = mvOpt(transMat, init_var, lambda, eMax, x, sigma_Lt, gammaL, numMics, numArrays, micsPos, t, p_fails, sub_p_hat_ts, scales, RTF_train,RTF_test, rirLen, rtfLen,sourceTrain, sourceTest, nL, nU, roomSize, T60, c, fs, kern_typ)
...
% ----Calculate new estimate based off movement for all subnets and resid. error from stationary time (turns off once estimates settle) ----
    ...
    for k1 = 1:numArrays
        ...
        [~,~,~,upd_sub_p_hat_ts(k1,:)] = test(x, sigma_Lt, gammaL, trRTF, RTF_test, subnet, rirLen, rtfLen, numArrays-1, numMics, sourceTrain, sourceTest, nL, nU, roomSize, T60, c, fs, kern_typ, subscales);   
    end
