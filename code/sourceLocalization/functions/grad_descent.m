function [tot_cost, vari, scales, varis_set, scales_set] = grad_descent(sourcePos, numArrays, rtfs, K_init, cov_init, scales_init, var_init, max_iters, tol, alpha, kern_typ)
    ...
    while or(and(mean(costs)>=tol, niter <= max_iters), mse(prev_cost,mean(costs))>=tol)
        ...
        dL_dScales = scale_grad(sourcePos, rtfs(1:nL,:,:), new_Ks, numArrays, new_gamma, kern_typ, new_scales);
        dL_dVar = var_grad(sourcePos, new_gamma);
        
        new_scales = new_scales - alpha.*dL_dScales;
        new_vari = new_vari - alpha*dL_dVar;
        
        [~,new_cov,new_Ks] = trCovEst(nL, nD, numArrays, rtfs, kern_typ, new_scales);
        new_varI = new_vari*eye(nL);
...
function dL_dScales = scale_grad(source_pos, rtfs, Ks, numArrays, gamma, kern_typ, scales)
