function [upd_sub_p_hat_ts, prob_failure, posteriors] = moveDetectorDemo(rtfDatabase, gammaL, numMics, numArrays, micsPos, t, p_fails, sub_p_hat_ts, scales, RTF_train, rirLen, rtfLen,sourceTrain, sourceTest, nL, nU, roomSize, T60, c, fs, kern_typ)
...
% ----Calculate new estimate based off movement for all subnets and resid. error from stationary time (turns off once estimates settle) ----
    ...
    for k1 = 1:numArrays
        ...
        [~,~,upd_sub_p_hat_ts(k1,:)] = testDemo(rtfDatabase, gammaL, trRTF, micsPos, subnet, numArrays-1, sourceTrain, nL, nU, kern_typ, subscales);   
    end
