...
load('./mat_results/loclErrorFullNet_res', 'localErrors', 'radii')
localError = mean(localErrors,2);
figure(2)
bar([0;localError])
title(sprintf('Average Difference in Network Source Estimation Before and After Movement\n (100 Trials per Threshold Simulated w/Varying Array Shifts)\n[Shifts: 0.05m - 3.05m by 20cm increments]'))
xlabel(sprintf('Array Shift (m) from Location Before Movement'))
ylabel(sprintf('Estimation Difference (m) After Shift'))
mz = {[0 round(radii(1:2:end),2)]};
xticklabels(mz)
% set(gca,'XTickLabel',mz)
ylim([0 1])
