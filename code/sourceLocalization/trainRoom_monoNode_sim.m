...
scales = zeros(numArrays,numMics);
varis = zeros(numArrays, numMics);
gammaLs = zeros(numArrays, nL, nL);
for t = 1:numArrays
    ...
    RTF_curr = reshape(RTF_train(t,:,:,:), [nD, rtfLen, numMics]);
    ...
    scales(t,:) = scales_set(I,:);
    vari(t,:) = varis_set(I,:);
    [~,sigmaL] = trCovEst(nL, nD, numMics, RTF_curr, kern_typ, scales(t,:));
    gammaLs(t,:,:) = inv(sigmaL + diag(ones(1,nL).*vari(t,:)));
    
end
save('mat_outputs/biMicCircle_5L300U_monoNode')
