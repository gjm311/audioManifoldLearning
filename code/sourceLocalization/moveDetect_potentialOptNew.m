...
%     init_transMat = [.65 0 0.35; .2 0 0.8; 1/3 1/3 1/3];
    ...
    pEst_mis = 0;
        ...
        emp_rows = [1 1];
        emp_cols = [pEmp_aa+pEmp_ma; pEmp_mm+pEmp_am; 2/3];
        transMat = ones(2,3);
        pairity = 1;
        rows = 0;
        cols = 0;
        while (isequal(emp_rows,rows)+isequal(emp_cols,cols))~=2
            rows = emp_rows;
            cols = emp_cols;
            if mod(pairity,2) == 1
                transMat = transpose([cols cols]).*(transMat./sum(transMat,1));
                rows = sum(transMat,2);
            else
                transMat = [transpose(rows) transpose(rows) transpose(rows)].*(transMat./sum(transMat,2));
                cols = sum(transMat,1);
            end
            pairity = pairity+1;
        end
    ...
    transMat_t = [transMat;ones(1,3).*(1/3)];
